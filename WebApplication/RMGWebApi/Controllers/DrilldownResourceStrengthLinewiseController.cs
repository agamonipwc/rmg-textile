...
namespace RMGWebApi.Controllers
{
    ...
    [ApiController]
    public class DrilldownResourceStrengthLinewiseController : Controller
    {
        private readonly RepositoryContext _rmgDbContext;
        public DrilldownResourceStrengthLinewiseController(RepositoryContext rmgDbContext)
        {
        ...
        public JsonResult Post(KPIViewModel kpiViewModel)
        {
            var kpiResults = new
            {
                ProductionLineWiseAnalysis = CalculateOperatorsLineWise(kpiViewModel),
                DHULineWiseAnalysis = CalculateHelpersDataLineWise(kpiViewModel),
                RejectionLineWiseAnalysis = CalculateCheckersDataLineWise(kpiViewModel),
                StatusCode = 200
            };
            return Json(kpiResults);
        }

        private JsonResult CalculateOperatorsLineWise(KPIViewModel kpiViewModel)
        {
            var productionDataGroupingByLineDay = _rmgDbContext.OperatorNos.Where(x =>
                ...
                .Select(grp => new OperatorViewModel { OperatorData = grp.Sum(c => c.Data), Day = grp.Key.Day, Line = grp.Key.Line, Month = grp.Key.Month }).ToList();
        ...
        private JsonResult CalculateHelpersDataLineWise(KPIViewModel kpiViewModel)
        {
            var dhuDataGroupingByLineDay = _rmgDbContext.Helpers.Where(x =>
                kpiViewModel.Year.Contains(x.Date.Year) &&
                kpiViewModel.Line.Contains(x.Line) && kpiViewModel.Month.Contains(x.Date.Month)).GroupBy(x => new { x.Day, x.Line })
                .Select(grp => new HelpersViewModel { HelperData = grp.Sum(c => c.Data), Day = grp.Key.Day, Line = grp.Key.Line }).ToList();
        ...
        private JsonResult CalculateCheckersDataLineWise(KPIViewModel kpiViewModel)
        {
            var rejectionDataGroupingByLineDay = _rmgDbContext.Checkers.Where(x =>
                kpiViewModel.Year.Contains(x.Date.Year) &&
                kpiViewModel.Line.Contains(x.Line) && kpiViewModel.Month.Contains(x.Date.Month)).GroupBy(x => new { x.Day, x.Line, x.Date.Month })
                .Select(grp => new CheckersViewModel { CheckersData = grp.Sum(c => c.Data), Day = grp.Key.Day, Line = grp.Key.Line, Month = grp.Key.Month }).ToList();
