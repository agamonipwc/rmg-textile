...
namespace RMGWebApi.Controllers
{
    ...
    [ApiController]
    public class DrilldownProductivityUnitwiseController : Controller
    {
        private readonly RepositoryContext _rmgDbContext;
        public DrilldownProductivityUnitwiseController(RepositoryContext rmgDbContext)
        {
        ...
        public JsonResult Post(KPIViewModel kpiViewModel)
        {
            var kpiResults = new
            {
                ProductionUnitWiseAnalysis = CalculateWIPDataUnitWise(kpiViewModel),
                DHUUnitWiseAnalysis = CalculateProductionDataUnitWise(kpiViewModel),
                RejectionUnitWiseAnalysis = CalculateWorkingHrsDataUnitWise(kpiViewModel),
                StatusCode = 200
            };
            return Json(kpiResults);
        }

        private JsonResult CalculateWIPDataUnitWise(KPIViewModel kpiViewModel)
        {
            var productionDataGroupingByUnitDay = _rmgDbContext.WIP.Where(x =>
                ...
                .Select(grp => new WIPViewModel { WIPData = grp.Sum(c => c.Data), Day = grp.Key.Day, Unit = grp.Key.Unit, Month = grp.Key.Month }).ToList();
        ...
        {
            var dhuDataGroupingByUnitDay = _rmgDbContext.Production.Where(x =>
                kpiViewModel.Year.Contains(x.Date.Year) &&
                kpiViewModel.Unit.Contains(x.Unit) && kpiViewModel.Month.Contains(x.Date.Month)).GroupBy(x => new { x.Day, x.Unit })
                .Select(grp => new ProductionViewModel { ProdData = grp.Sum(c => c.Data), Day = grp.Key.Day, Unit = grp.Key.Unit }).ToList();
        ...
        private JsonResult CalculateWorkingHrsDataUnitWise(KPIViewModel kpiViewModel)
        {
            var rejectionDataGroupingByUnitDay = _rmgDbContext.WorkingHrs.Where(x =>
                kpiViewModel.Year.Contains(x.Date.Year) &&
                kpiViewModel.Unit.Contains(x.Unit) && kpiViewModel.Month.Contains(x.Date.Month)).GroupBy(x => new { x.Day, x.Unit, x.Date.Month })
                .Select(grp => new WorkingHoursViewModel { WorkingHrsData = grp.Sum(c => c.Data), Day = grp.Key.Day, Unit = grp.Key.Unit, Month = grp.Key.Month }).ToList();
