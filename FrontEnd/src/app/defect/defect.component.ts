...
export class DefectComponent implements OnInit {
  ...
  getFilterData(){
    ...
    if($('#startDate').val() == $('#endDate').val()){
      this.headerTextValue = environment.defectHeaderText + " on " + userFormattedDateOutput["startDateTime"];
    }
    else{
      this.headerTextValue = environment.defectHeaderText + " from " + userFormattedDateOutput["startDateTime"] + " to " + userFormattedDateOutput["endDateTime"];
    }
  }
  ...
  getRecommendation(recommendationId){
    ...
    this.http.post<any>(url, recommendationView).subscribe(responsedata =>{
      _this.recommendationModalTitle = "Recommendations to improve defect%"
  ...
  ExportToExcelLowEfficiency() {  
    ...
    XLSX.writeFile(wb, 'Low_Defect%.xlsx');  
  }  
  ExportToExcelLowOperators(){
    ...
    const wb: XLSX.WorkBook = XLSX.utils.book_new();  
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');  
    XLSX.writeFile(wb, 'Low_Defect%_Operators.xlsx');  
  }
  ExportToExcelModerateEfficiency(){
    ...
    const wb: XLSX.WorkBook = XLSX.utils.book_new();  
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');  
    XLSX.writeFile(wb, 'Moderate_Defect%.xlsx');  
  }
  ExportToExcelModerateOperators(){
    ...
    const wb: XLSX.WorkBook = XLSX.utils.book_new();  
    XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');  
    XLSX.writeFile(wb, 'Moderate_Defect%_Operators.xlsx');  
  }
